...
@section('content')
    ...
    <div class="col-sm-12 d-flex gap-4">
        <a href="{{ route('admin.districtcommitteedetails.file') }}">
            <button class="btn-primary btn-sm">
                <i class="fa fa-plus"></i>
                Import
            </button>
        </a>
        <a href="{{ route('admin.districtcommitteedetails.create') }}">
            <button class="btn-primary btn-sm">
                <i class="fa fa-plus"></i>
                Add New Member
            </button>
        ...
        <form action="{{ route('admin.districtcommitteedetails.filee-export-exe') }}" method="POST" class="d-flex gap-2">
            @csrf
            ...
            <button type="submit" class="btn-sm btn-primary" style="height: 30px;">Export District Committee Details</button>
        </form>
    </div><!-- /.col -->
    ...
    <form action="{{ route('admin.districtcommitteedetails.index') }}" method="GET" class="mb-4 mt-4">
        <div class="form-group">
            <label for="province">Select District for Viewing:</label>
            <select name="district_id" id="district" class="form-control" style="width: 700px;">
    ...
    <table class="table table-bordered table-hover">
        ...
        <tbody>
            @foreach ($districtcommitteedetails as $ed)
                <tr data-widget="expandable-table" aria-expanded="false">
                    <td>{{ $loop->iteration }}</td>
                    <td>{{ $ed->name ?? '' }}</td>
                    <td>{{ $ed->phone }}</td>
                    <td>
                        <img id="preview" src="{{ url('uploads/districtcommitteedetail/' . $ed->image) }}"
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            ...
                            <a href="{{ url('admin/districtcommitteedetails/destroy/' . $ed->id) }}">
            ...
            <div class="modal fade" id="edit{{ $ed->id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-danger" data-bs-dismiss="modal">No</button>
                            <a href="{{ url('admin/districtcommitteedetails/edit/' . $ed->id) }}">
                                <button type="button" class="btn btn-danger">Yes
                                </button>
                            </a>
                        </div>
